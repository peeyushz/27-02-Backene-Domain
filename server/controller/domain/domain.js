            ...
            if (domainExists === 0) {
                ...
                if (receivedData.domain.includes("/") || receivedData.domain.includes(":")) {
                    ...
                    if (userData && purchaseDetails) {
                        ...
                        const address = await contract.methods.ownerOf("0x"+labelHash).call();
                        const expiryDate = await contract.methods.nameExpires("0x"+labelHash).call();
                        if (address && expiryDate) {
                            const newDomainData = new domainModel({
                                ...
                                alertTime: (receivedData.alertTime ? receivedData.alertTime : "12:00"),
                                alertDate: receivedData.alertDate,
...
exports.updateDomainData = async (req,res) => {
    try {
        ...
        let areNotificationsEnabled = null; let alertTime = null; let alerts = null; let alertDate = null;
            ...
            await domainModel.findOne({ _id: domainId }).then(async(domainData) => {
                if (domainData) {
                    ...
                    if (alertDate === null) {
                        alertDate = domainData.alertDate;
                    }
                    if (alerts === null) {
                        alerts = domainData.alerts;
                    }
                    let updateTime = new Date()
                    await domainModel.findOneAndUpdate({ _id: domainId }, { $set: { areNotificationsEnabled: areNotificationsEnabled, alertTime: alertTime, alertDate: alertDate, alerts: alerts, updatedAt: updateTime } }, { upsert: false }).then((updatedRecord) => {
                        if (updatedRecord) {
